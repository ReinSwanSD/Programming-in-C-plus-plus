...
void htable::fillTable(istream& fin)
{
  ...
  while(fin >> akey)  // IF I CAN READ THE NAME
    { 
      fin >> aname;
      fin >> agender;
      fin >> amajor;
      
      el_t anelement(akey,aname,agender,amajor);
      
      s=hash(akey);
      table[s].addRear(anelement);
    }
